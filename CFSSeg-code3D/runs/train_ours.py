        ...
        if step == 0: # Train the base model on the base classes
            ...
            for epoch in range(args.n_epochs):
                for batch_idx, (ptclouds, labels) in enumerate(TRAIN_LOADER):
                    ...
                    pc_relations_new, logits_new = model_new(ptclouds, relation=True)
                    ...
                    index_probs, uncertain_old, uncertain_knn = model_old.uncertainty_estimation(ptclouds[:, :3, :].transpose(1, 2), pred_old.transpose(1, 2))
                    ...
                    if args.dataset == 'scannet':
                        labels_new = torch.where(labels_new_cls==0, -1, labels_new_cls) # For scannet ignore labels 0 -> -1 : unannotated
                        labels_new_cls = labels_new

                    loss_cls = F.cross_entropy(logits_new_cls, labels_new_cls.cuda(), ignore_index=-1)

                    # Geometry-aware Feature-Relation Transfer
                    loss_struct_distill = torch.square(pc_relations_new - pc_relations_old).mean([2, 1, 0])
                    # del labels, logits_new, logits_old, logits_old_cls, pred_old, index_probs
                    loss_incre = loss_cls + loss_struct_distill
            ...
            if step == 1:
                ...
                model_new = DGCNNSeg(args)
                classifer_weights_old = torch.empty_like(classifer_old.classifer_weights).copy_(classifer_old.classifer_weights.detach())
                classifer_new = Classifer(num_classes=NUM_BASE_CLASSES + step + 1, # Add a background class
                                          initial_old_classifer_weights=classifer_weights_old)
                model_new = load_trained_checkpoint(model_new, args.base_model_checkpoint_path,
                                                    'end_base_model_checkpoint.tar')  # Load the last base model
            else:
                # Init the old model
                model_old = DGCNNSeg(args, option='uncertain')
                model_old = load_trained_checkpoint(model_old, args.base_model_checkpoint_path,
                                                    'end_incre_step_' + str(step - 1) + '_model_checkpoint.tar')   # Load the last base model
                classifer_old = Classifer(num_classes=NUM_BASE_CLASSES + step)
                classifer_old = load_trained_checkpoint(classifer_old, args.base_model_checkpoint_path,
                                                        'end_incre_step_' + str(step - 1) + '_model_classifer_checkpoint.tar')
                model_new = DGCNNSeg(args)
                classifer_weights_old = torch.empty_like(classifer_old.classifer_weights).copy_(classifer_old.classifer_weights.detach())

                # --------------------------------------------------------------------------------------------------------------------
                if step != STEP - 1:
                    classifer_new = Classifer(num_classes=NUM_BASE_CLASSES + step + 1, initial_old_classifer_weights=classifer_weights_old)
                else: # The end step do not need the background
                    classifer_new = Classifer(num_classes=NUM_BASE_CLASSES + step, initial_old_classifer_weights=classifer_weights_old)
            ...
            for epoch in range(args.n_epochs):
                for batch_idx, (ptclouds, labels) in enumerate(TRAIN_LOADER):
                    ...
                    if step != STEP - 1: # Involve the background
                        labels_new = torch.where(labels == 0, pred_old.argmax(dim=1),
                                                 labels + int(NUM_BASE_CLASSES) + INCRE * step - int(1.0))
                    else: # Do not involve the background
                        labels_new = torch.where(labels == 0, pred_old.argmax(dim=1) - int(1.0),
                                                 labels + int(NUM_BASE_CLASSES) + (INCRE*step - int(1.0)) - int(1.0))
                    ...
                    for k in range(index_probs.shape[2]):
                        if (labels_new_cls == -1).long().sum([0, 1]) >= 1:
                            if k == 0:
                                labels_new_cls = torch.where(
                                    (labels_new_cls == -1) | (uncertain_old > args.uncertain_t),
                                    index_probs[:, :, k], labels_new_cls)
                            else:
                                labels_new_cls = torch.where((labels_new_cls == -1),
                                                             index_probs[:, :, k], labels_new_cls)
                    ...
                    if args.dataset == 'scannet':
                        if step == STEP - 1:
                            labels_new = torch.where(labels_new_cls == 0, -1, labels_new_cls)
                            labels_new_cls = labels_new
                ...
                if (epoch + 1) % args.eval_interval == 0:
                    ...
                    gt_total = []
                    with torch.no_grad():
                        for iter, (ptclouds, labels) in enumerate(VALID_LOADER):
                            if torch.cuda.is_available():
                                ptclouds = ptclouds.cuda()
                                labels = labels.cuda()
                            model_new.eval()
                            ...
                            if step != STEP - 1:
                                logits_new_cls_eval = torch.zeros([B, 1 + INCRE, N]).cuda()
                                logits_new_cls_eval[:, 0, :] = torch.sum(
                                    logits_new_cls[:, :NUM_BASE_CLASSES + step * INCRE, :], dim=1)
                                logits_new_cls_eval[:, 1:, :] = logits_new_cls[:, NUM_BASE_CLASSES + step * INCRE:, :]
                            else:
                                logits_new_cls_eval = torch.zeros([B, 1 + INCRE, N]).cuda()
                                logits_new_cls_eval[:, 0, :] = torch.sum(
                                    logits_new_cls[:, :NUM_BASE_CLASSES + step * INCRE - 1, :], dim=1)
                                logits_new_cls_eval[:, 1:, :] = logits_new_cls[:, NUM_BASE_CLASSES + step * INCRE - 1:, :]
                            ...
                            WRITER.add_scalar('Valid/loss', loss_incre, global_iter)
                            logger.cprint(
                                '=====[Valid] Epoch: %d | Iter: %d | Loss: %.4f =====' % (epoch, iter, loss_incre.item()))

                    pred_total = torch.stack(pred_total, dim=0).view(-1, args.pc_npts)
                    gt_total = torch.stack(gt_total, dim=0).view(-1, args.pc_npts)

                    _, mIoU, iou_perclass = metric_evaluate(pred_total, gt_total, 2,
                                                            2, 'eval_Incre', range(1, len(CURRENT_CLASS)+1), logger, args.dataset)
                    ...
                    if mIoU > best_iou:
                        best_iou = mIoU
                        logger.cprint('*******************Model Saved*******************')
                        save_train_checkpoint(model_new, args.log_dir, 'best_incre_step_'+str(step)+'_model')
                        save_classifer_checkpoint(classifer_new, args.log_dir, 'best_incre_step_'+str(step)+'_model')

            logger.cprint('*******************End of Training the Incre Model at Step %d*******************' %step)
            logger.cprint('*******************Eval the Incre Model*******************')
            pred_end_total = []
            gt_end_total = []
            with torch.no_grad():
                for iter, (ptclouds, labels) in enumerate(VALID_LOADER):
                    ...
                    if step != STEP - 1:
                        logits_new_cls_eval = torch.zeros([B, 1 + INCRE, N]).cuda()
                        logits_new_cls_eval[:, 0, :] = torch.sum(logits_new_cls[:, :NUM_BASE_CLASSES+step*INCRE, :], dim=1)
                        logits_new_cls_eval[:, 1:, :] = logits_new_cls[:, NUM_BASE_CLASSES+step*INCRE:, :]
                    else:
                        logits_new_cls_eval = torch.zeros([B, 1 + INCRE, N]).cuda()
                        logits_new_cls_eval[:, 0, :] = torch.sum(logits_new_cls[:, :NUM_BASE_CLASSES+step*INCRE-1, :], dim=1)
                        logits_new_cls_eval[:, 1:, :] = logits_new_cls[:, NUM_BASE_CLASSES+step*INCRE-1:, :]
                    ...
                    WRITER.add_scalar('Valid/loss', loss_incre, global_iter)
                    logger.cprint('=====[Valid] End | Iter: %d | Loss: %.4f =====' % (iter, loss_incre.item()))
