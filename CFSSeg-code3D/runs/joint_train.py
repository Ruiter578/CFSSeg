...
def train(args):
    ...
    #         if label < len(train_class_counts):
    #             train_class_counts[label] += count

    # # 统计验证集中每个类别的样本数量
    # valid_class_counts = [0] * len(CLASSES)
    # for i in range(len(VALID_DATASET)):
    #     _, labels = VALID_DATASET[i]
    #     unique_labels, counts = torch.unique(labels, return_counts=True)
    #     for label, count in zip(unique_labels.cpu().numpy(), counts.cpu().numpy()):
    #         if label < len(valid_class_counts):
    #             valid_class_counts[label] += count

    # # 打印每个类别的样本数量
    # logger.cprint('=== Train Dataset Class Distribution ===')
