...
def freeze_and_add(args):
    ...
    for step in range(0, STEP): # 0 for base step, 1~STEP for incre step
        ...
        if step == 0: # Train the base model on the base classes
            ...
            logger.cprint('*******************Freeze and add the Model on Incremental Classes: %d Classes | Total 2 Tasks | Step %d'
            ...
            optimizer_incre = optim.Adam([{'params': classifer_new.parameters(), 'lr': args.incre_lr}], \
