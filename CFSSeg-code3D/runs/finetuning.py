...
def finetuning(args):
    ...
    # If you already have a trained base model, you can put the base checkpoint in the corresponding folder, and you only \
    ...
    for step in range(0, STEP): # 0 for base step, 1~STEP for incre step
        SAMPLE_CLASS = BASE_CLASSES.copy()
        if step == 0: # Train the model on base classes
            CLASSES = BASE_CLASSES
            CURRENT_CLASS = BASE_CLASSES
            LOG_DIR = args.log_dir + '/base_model'
        else: # Train the model on incremental classes
        ...
        if step == 0: # Train the base model on the base classes
            ...
            logger.cprint('*******************Finetuning the Model on Incremental Classes: %d Classes | Total 2 Tasks | Step %d'
                          '*******************' % (len(INCRE_CLASSES), step))
            # Train the model for incremental classes: step > 0 and tasks == 2
            model_new = DGCNNSeg(args)
            classifer_new = Classifer(num_classes=NUM_ALL_CLASSES - NUM_BASE_CLASSES + 1) # Incre classes number + 1 background
            ...
            _, mIoU, iou_perclass = metric_evaluate(pred_total, gt_total, 2,
                                                    2, 'eval_Incre',
                                                    range(1, len(CURRENT_CLASS)+1), logger, args.dataset)
            logger.cprint('===== End Model | mIoU: %f =====\n' % (mIoU))
            logger.cprint('*******************Model Saved*******************')
