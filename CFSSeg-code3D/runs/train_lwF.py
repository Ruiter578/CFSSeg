    ...
    for step in range(1, STEP): # 0 for base step, 1~STEP for incre step
        ...
        if step == 0: # Train the base model on the base classes
            ...
            if step == 1:
                ...
                classifer_old = Classifer(num_classes=NUM_BASE_CLASSES + step-1)
                classifer_old = load_trained_checkpoint(classifer_old, args.base_model_checkpoint_path,
                ...
                # if step != STEP - 1:
                #     classifer_new = Classifer(num_classes=NUM_BASE_CLASSES + step + 1, initial_old_classifer_weights=classifer_weights_old,save_backgroudclass=True)
                # else: # 最后一步不需要背景类
                classifer_new = Classifer(num_classes=NUM_BASE_CLASSES + step, initial_old_classifer_weights=classifer_weights_old,save_backgroudclass=True)
            ...
            for epoch in range(args.n_epochs):
                for batch_idx, (ptclouds, labels) in enumerate(TRAIN_LOADER):
                    ...
                    _, logits_old = model_old(ptclouds)
                    logits_old_cls = classifer_old(logits_old)
                    # print('logits_new_cls:',logits_new_cls.shape)
                    # print('logits_old_cls:',logits_old_cls.shape)
                    # if step != STEP - 1:
                    ...
                    # if step != STEP - 1:
                    #     output_new_logits = F.softmax(logits_new_cls.transpose(1, 2)[:, :, 1:NUM_BASE_CLASSES+(step-1)+1] / T, dim=-1)
                    #     output_old_logits = F.softmax(logits_old_cls.transpose(1, 2)[:, :, 1:] / T, dim=-1)
                    # else:
                ...
                if (epoch + 1) % args.eval_interval == 0:
                    ...
                    with torch.no_grad():
                        for iter, (ptclouds, labels) in enumerate(VALID_LOADER):
                            ...
                            #         logits_new_cls[:, :NUM_BASE_CLASSES + (step - 1) * INCRE+1, :], dim=1)
                            #     logits_new_cls_eval[:, 1:, :] = logits_new_cls[:, NUM_BASE_CLASSES + (step - 1) * INCRE+1:, :]
                            # else:
                            logits_new_cls_eval = torch.zeros([B, 1 + INCRE, N]).cuda()
                            logits_new_cls_eval[:, 0, :] = torch.sum(
                                logits_new_cls[:, :NUM_BASE_CLASSES + (step - 1) * INCRE, :], dim=1)
                            logits_new_cls_eval[:, 1:, :] = logits_new_cls[:, NUM_BASE_CLASSES + (step - 1) * INCRE:, :]
