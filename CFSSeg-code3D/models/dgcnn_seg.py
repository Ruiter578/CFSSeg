...
class Classifer(nn.Module):
    def __init__(self, num_classes, initial_old_classifer_weights=None,save_backgroudclass=False):
        ...
        if initial_old_classifer_weights != None and save_backgroudclass==False: # Only initialize the novel weights
        ...
        elif initial_old_classifer_weights != None and save_backgroudclass==True: # Only initialize the novel weights
            num_base_class, _, _ = initial_old_classifer_weights.size()
            print('num_classes',num_classes)
            print('num_base_class',num_base_class)
            print('num_classes-num_base_class',num_classes-num_base_class)
            classifer_weights_novel = torch.empty(num_classes - num_base_class, 128, 1).cuda()
            if num_classes-num_base_class!=0:
                torch.nn.init.kaiming_normal_(classifer_weights_novel)
                classifer_weights = torch.cat((initial_old_classifer_weights[0:, :, :], classifer_weights_novel), dim=0)
                self.classifer_weights = nn.Parameter(classifer_weights, requires_grad=True)
            else:
                self.classifer_weights = nn.Parameter(initial_old_classifer_weights, requires_grad=True)
