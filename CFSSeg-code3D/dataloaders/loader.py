"""Data Loader for Incremental Tasks"""
...
class MyDataset(Dataset):
    def __init__(self, data_path, classes, curr_classes, class2scans, step, mode='train', valid_set='Area_5',
        ...
        all_block_names = []
        if isinstance(curr_classes, int):
            self.curr_classes = list([curr_classes])
        else:
            self.curr_classes = list(curr_classes)

        for sampled_class in self.curr_classes:
            all_scannames = self.class2scans[sampled_class].copy()
            all_block_names.extend(all_scannames)
