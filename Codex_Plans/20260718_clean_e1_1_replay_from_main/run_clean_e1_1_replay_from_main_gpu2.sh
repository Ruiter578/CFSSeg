...
RUN_ID="${RUN_ID:-20260718_clean_e1_1_replay_from_main_gpu2}"
...
manifest_matches_current() {
    ...
    [[ "$(jq -r '.analytic_tail_epsilon | tostring' "$manifest")" == "$(normalize_float "$ANALYTIC_TAIL_EPSILON")" ]] || return 1
...
run_one() {
    ...
    result="$(latest_val_result "$step_dir")"
    if [[ -z "$result" ]] || ! manifest_matches_current "$manifest" "$source" "$buffer" "$rhl_seed"; then
...
write_summary() {
    ...
    printf 'feature_source\tbuffer\trhl_seed\tall_miou\told_0_15_miou\tnew_16_20_miou\tgit_dirty\tgit_commit\tsubpath\tresult_json\n' > "$output"
...
echo "[INFO] clean replay head=$HEAD_AT_LAUNCH run_id=$RUN_ID primary_root=$PRIMARY_ROOT"
