...
> **状态：W0、weighted C-RLS 实现与 W1 四组实验均已完成；W1 未通过预注册 gate，路线已停止，不启动 W2/W3。**
>
> W1 正式结果与路线决策见 `7-19_W1可靠性加权C-RLS正式结果与路线决策.md`。后续不得继续 seed2/3、alpha、floor、temperature 或阈值小网格；若重启 soft-target 方向，必须作为新的研究假设重新设计和预注册。
