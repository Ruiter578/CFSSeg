...
做 RHL、伪标签、集成学习或论文主线实验设计时，必须在第一性原理和对抗性审查之后执行下面提示：

```text
不要把"最小改动"误当成"最佳研究路线"。SegACIL 的方法探索要同时给出三条线：保守验证线用于证明代码和协议没错，进取探索线用于主动搜索更强机制和更宽参数空间，论文级证据线用于对齐 CFSSeg 原论文协议、主表和消融。设计实验时不要只启动一个最小 next run；应在显存和时间允许时组织成矩阵、分阶段筛选或跨服务器并行队列，并明确每组实验能推翻或支持哪个机制假设。若现有 runner、summary 或 audit 工具限制了探索质量，应新增干净工具，而不是为了少改代码牺牲可复现性、可解释性或论文表达力。
```
