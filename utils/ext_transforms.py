...
class ExtResize(object):
    ...
    def __call__(self, img, lbl):
        return F.resize(img, self.size, interpolation=InterpolationMode.BILINEAR), \
                F.resize(lbl, self.size, interpolation=InterpolationMode.NEAREST)
...
class ExtRandomScale(object):
    """Randomly resize image and label by a scale factor sampled from scale_range."""

    def __init__(self, scale_range):
        if not isinstance(scale_range, (tuple, list)) or len(scale_range) != 2:
            raise TypeError("scale_range should be a tuple/list with length 2.")
        if scale_range[0] <= 0 or scale_range[1] <= 0:
            raise ValueError("scale_range values must be positive.")
        if scale_range[0] > scale_range[1]:
            raise ValueError("scale_range[0] should be <= scale_range[1].")
    ...
    def __call__(self, img, lbl):
        ...
        w, h = img.size
        target_size = (max(1, int(round(h * scale))), max(1, int(round(w * scale))))

        return F.resize(img, target_size, interpolation=InterpolationMode.BILINEAR), \
