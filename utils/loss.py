...
class BCEWithLogitsLossWithIgnoreIndex(nn.Module):
    ...
    def forward(self, inputs, targets, weight=None) -> torch.Tensor:
...
def build_criterion(opts : Config):
    # Set up criterion
    if opts.loss_type == 'focal_loss':
        ...
        return BCEWithLogitsLossWithIgnoreIndex(ignore_index=255, reduction='mean')
    else:
        raise ValueError(f"Unsupported loss type: {opts.loss_type}")
